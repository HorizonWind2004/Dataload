...
def check_on_image(image = None,prompt = None,gt_bbox = None,attr = None,box_t = 0.25,text_t = 0.25,miou_threshold = 0.5,args = None,image_path = None):
    ...
    # labels = [
    #     f"{CLASSES[detections.class_id[i]]} {detections.confidence[i]:0.2f}" for i in range(len(detections.class_id))
    # ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path',type=str,default='../MIGC/bench_file/mig_bench.json')
    
    args = parser.parse_args()
    miou_threshold = args.miou_threshold

    if args.need_clip_score:
        clip_model = CLIPModel.from_pretrained('../../ckpt/clip-vit-large-patch14').cuda().eval()
        clip_processor = CLIPProcessor.from_pretrained('../../ckpt/clip-vit-large-patch14')
    ...
    metric_result = {}
    bench_set = set()
    with open('../MIGC/bench_file/mig_bench_tpami.txt','r') as f:
        for line in f:
            bench_set.add(line.strip())
        
    for image_path in tqdm(image_path_list):
        if 'anno_' in image_path:
            # print(f'skip the {image_path}')
            continue
        ...
        prompt = image_path.split('_')[0]
        
        if prompt not in bench_set:
            print(f'skip the {image_path}')
            continue
